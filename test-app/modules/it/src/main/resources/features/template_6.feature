...
Feature: Testing a REST API with page template-6
  ...
  Scenario: Page template (template_6) uploaded to a web service
    ...
    And users provide page template file 'template_6.yaml'
  ...
  Scenario: Page templates retrieved from a web service
    ...
    Then page templates data 'template_6.json' should be successfully returned
  ...
  Scenario: Page template (template_6) rendered on a web service
    Given users want to view installed page template on a web page
    And users want to compare with file template 'template_6.yaml'
